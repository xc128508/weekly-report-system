...
app.use(
  session({
    secret: process.env.SESSION_SECRET || 'xieyou-weekly-report-demo-secret',
    resave: false,
...
function layout({ title, user, content }) {
  let navLinks = '';
  if (user?.role === 'admin') {
    navLinks = '<a href="/admin/dashboard">今日提交监督</a><a href="/admin/daily-tasks">每日任务</a><a href="/admin/task-pool">任务总表</a><a href="/admin/reports/generate">生成周报</a><a href="/admin/users">账号管理</a><a href="/change-password">修改密码</a>';
    ...
    navLinks = '<a href="/intern/dashboard">我的周报</a><a href="/intern/daily-tasks">每日任务</a><a href="/intern/task-pool">任务认领</a><a href="/intern/reports/new">填写周报</a><a href="/change-password">修改密码</a>';
  }
  const nav = user
    ? `<header class="topbar">
        <div>
          <div class="company">解悠数字科技（南京）有限公司</div>
...
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('table').forEach(function (table) {
    ...
    const isTaskPoolTable =
      ...
      headers.includes('状态') &&
      headers.some(h => h.includes('任务'));

    if (!isTaskPoolTable || table.dataset.actionMovedFirst === '1') return;
    ...
    function moveActionCellToFirst(row) {
      ...
      row.insertBefore(cells[actionIndex], cells[0]);
    }
    ...
    table.classList.add('task-pool-actions-first');
    table.dataset.actionMovedFirst = '1';
  });
});
</script>
...
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('table').forEach(function (table) {
    ...
    table.classList.add('daily-actions-first');
...
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('table').forEach(function (table) {
    ...
    const isTaskPoolTable =
      headers.includes('需求部门') &&
      headers.includes('对接人') &&
      headers.some(h => h.includes('任务标题'));

    if (!isTaskPoolTable || table.dataset.taskTitleClamped === '1') return;

    const titleIndex = headers.findIndex(h => h.includes('任务标题'));
    if (titleIndex < 0) return;

    table.querySelectorAll('tbody tr').forEach(function (row) {
      const cell = row.children[titleIndex];
      if (!cell || cell.querySelector('.task-title-clamp')) return;

      const text = cell.textContent.trim();
      cell.textContent = '';

      const span = document.createElement('span');
      span.className = 'task-title-clamp';
    ...
    table.classList.add('task-title-two-lines-table');
    table.dataset.taskTitleClamped = '1';
  });
});
</script>


<script id="xy-left-sidebar-layout-script">
document.addEventListener('DOMContentLoaded', function () {
  const hasSystemNav = document.querySelector(
    'a[href="/admin/daily-tasks"], a[href="/admin/task-pool"], a[href="/admin/reports/generate"], a[href="/admin/users"], a[href="/intern/daily-tasks"], a[href="/intern/reports"], a[href="/boss/reports"]'
  );

  if (!hasSystemNav) return;

  document.body.classList.add('xy-left-sidebar-layout');

  const header = document.querySelector('header') || document.querySelector('.topbar') || document.querySelector('.navbar');
  if (header) {
    header.classList.add('xy-left-sidebar');
  }

  const nav = header ? (header.querySelector('nav') || header.querySelector('.nav')) : null;
  if (nav) {
    nav.classList.add('xy-sidebar-nav');
  }

  const currentPath = window.location.pathname;
  document.querySelectorAll('.xy-sidebar-nav a').forEach(function (a) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('header strong, header .brand, .topbar strong, .topbar .brand, .navbar strong, .navbar .brand').forEach(function (el) {
    if (el.textContent.indexOf('解悠数字科技') !== -1) {
      el.classList.add('xy-company-name-one-line');
      el.textContent = '解悠数字科技（南京）有限公司';
    }
  });
...
<script src="/admin-layout-enhance.js?v=2026070203"></script>
<script src="/boss-dashboard-fix.js?v=2026070201"></script>
<script src="/admin-daily-table-layout.js?v=2026070203"></script>
<script src="/sidebar-bottom-font.js?v=2026070301"></script>
<script src="/intern-daily-multi-task.js?v=2026070308"></script>
...
function taskHasApprovedAssignee(task = {}) {
  return taskApprovedUserIds(task).length > 0 || taskApprovedNames(task).length > 0;
}

function taskApprovedByUser(task = {}, user = {}) {
  if (!user) return false;
  return taskApprovedUserIds(task).includes(user.id) || taskApprovedNames(task).includes(user.realName);
}
...
function setTaskPendingClaims(task, claims = []) {
  ...
  claims.forEach((claim) => {
    ...
    if (!clean.some((item) => (item.userId || item.realName) === key)) {
      clean.push({ userId, realName, at: claim.at || now() });
    }
  });
...
function taskSelectOptions(tasks, selectedTitle = '') {
  return tasks.map((task) => `<option value="${escapeHtml(task.title)}" ${task.title === selectedTitle ? 'selected' : ''}>${escapeHtml(task.title)}（${escapeHtml(task.status || '已认领')}）</option>`).join('');
}
...
function validateDailyTaskRequired(body, user = null, db = null) {
  ...
  if (user && db) {
    const allowed = claimedTasksForUser(db, user).some((task) => task.title === body.taskTitle);
    if (!allowed) return '关联任务只能选择该实习生已认领且管理员已同意的任务，请先到“任务认领”申请并等待管理员同意。';
  }
  return null;
}
...
function syncTaskPoolProgressFromDaily(db, dailyTask) {
  ...
  const task = db.taskPool.find((item) => item.title === dailyTask.taskTitle && taskOwnedByUser(item, dailyUser));
  if (!task) return;
...
function upsertWeeklyReportFromDraft(db, user, startDate, endDate, draft, status = 'submitted') {
  const currentTime = now();
  const existing = db.reports.find((r) => r.userId === user.id && r.weekStart === startDate && r.weekEnd === endDate);
  const reportData = {
    userId: user.id,
    weekStart: startDate,
    weekEnd: endDate,
    ...
    status,
    submittedAt: status === 'submitted' ? currentTime : null,
  ...
  const report = { id: id(), ...reportData, createdAt: currentTime };
...
function dailyTaskEditForm({ user, task = null, backUrl = '/admin/daily-tasks' }) {
  ...
  const claimedOptions = selectedIntern ? taskSelectOptions(claimedTasksForUser(db, selectedIntern), task?.taskTitle || '') : '';
  ...
  return layout({
    ...
    content: `<section class="page-title">
        <div><h1>${isEdit ? '编辑日报' : '新增日报'}</h1><p>管理员可以新增、修改任意实习生的日报内容；关联任务应选择该实习生已认领且审核通过的任务。</p></div>
      ...
      <section class="card">
        <form method="post" action="${formAction}" class="form grid-form">
          ...
          <div class="hint-box">如果切换了所属实习生，请保存前确认“关联任务”属于该实习生已认领任务。</div>
          <div class="two-cols">
          ...
          <label>整体进度${progressSelect('progress', task?.progress ?? '')}</label>
...
<body class="login-body">
  <section class="login-card">
    <div class="login-logo">解悠数字科技（南京）有限公司</div>
    <h1>实习生周报系统</h1>
    <p>请使用公司分配的账号和密码登录</p>
    ${error}
  ...
  const rows = reports.length
    ? reports
        .map((r) => `<tr>
            ...
            <td><a class="link-button" href="/intern/reports/${r.id}/edit">查看 / 修改</a></td>
          </tr>`)
        .join('')
    : '<tr><td colspan="5" class="empty">暂无周报，请先填写每日任务，再用 AI 生成周报草稿。</td></tr>';

  res.send(layout({
    title: '我的周报',
    user: req.user,
    content: `<section class="page-title">
        <div><h1>我的周报</h1><p>你好，${escapeHtml(req.user.realName)}，你可以先填写每日任务，再自动汇总成周报。</p></div>
        <div class="actions"><a class="secondary" href="/intern/task-pool">任务认领</a><a class="secondary" href="/intern/daily-tasks/new">填写今日任务</a><a class="primary" href="/intern/reports/new">填写周报</a></div>
      </section>
      ...
      <section class="card ai-card">
        <h2>自动周报助手</h2>
        <p class="muted">填写完一周的每日任务后，系统可自动把“今日工作内容、问题、明日计划、进度”整理成周报草稿。</p>
        <form method="get" action="/intern/reports/new" class="filter-form compact-filter">
          <input type="date" name="weekStart" value="${weekStartDefault()}" required />
          <input type="date" name="weekEnd" value="${weekEndDefault()}" required />
...
function dailyTaskForm({ user, task = null }) {
  ...
  const options = taskSelectOptions(claimedTasks, task?.taskTitle || '');
  const noClaimedTaskTip = claimedTasks.length
    ? ''
    : '<div class="alert error">你还没有管理员审核通过的已认领任务，请先到“任务认领”申请任务，管理员同意后才能填写日报。</div>';
  return layout({
    title: isEdit ? '修改每日任务' : '填写每日任务',
    user,
    content: `<section class="page-title">
        <div><h1>${isEdit ? '修改每日任务' : '填写每日任务'}</h1><p>每个字段均为必填；关联任务只能从管理员已同意的认领任务中选择。</p></div>
...
app.get('/intern/task-pool', requireLogin, requireIntern, (req, res) => {
  const db = readDb();
  const visibleTasks = db.taskPool.filter((task) => taskVisibleToIntern(task, req.user)).sort((a, b) => taskSortWeight(a) - taskSortWeight(b) || String(a.expectedDate || '').localeCompare(String(b.expectedDate || '')));
  const rows = visibleTasks.length
    ? visibleTasks.map((task) => {
        const status = taskStatusLabel(task);
        let action = '<span class="muted">-</span>';
        if (userHasPendingClaim(task, req.user)) {
          action = `<span class="badge draft">等待管理员审核</span>
            <form method="post" action="/intern/task-pool/${task.id}/cancel-claim" class="inline-form">
              <button class="ghost small" type="submit">撤销申请</button>
            </form>`;
        } else if (isTaskUnclaimed(task)) {
          action = `<form method="post" action="/intern/task-pool/${task.id}/claim" class="inline-form">
            <button class="primary small" type="submit">申请认领</button>
          </form>`;
        } else if (taskApprovedByUser(task, req.user)) {
          action = '<span class="badge approved">我的任务</span>';
        }
        return `<tr>
    ...
    : '<tr><td colspan="8" class="empty">暂无可认领任务。管理员同意认领前，任务仍会向所有实习生开放。</td></tr>';

  res.send(layout({
    title: '任务认领',
    user: req.user,
    content: `<section class="page-title">
        <div><h1>任务认领</h1><p>你可以申请认领未分配任务。申请后可撤销；管理员未同意前任务持续开放。管理员同意后也可选择继续开放或关闭认领。</p></div>
        <a class="ghost-link" href="/intern/daily-tasks">返回每日任务</a>
      </section>
      <section class="card wide-card">
        <h2>任务总表</h2>
        <p class="muted">这里只显示“未认领任务”和“与你有关的任务”。管理员同意后可选择继续开放或关闭认领；关闭后任务才会对其他实习生隐藏。</p>
        <table class="task-pool-table">
          <thead><tr><th>需求部门</th><th>对接人</th><th>任务标题</th><th>期望完成日期</th><th>状态</th><th>认领/申请人</th><th>操作</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </section>`
  }));
...
<script src="/admin-taskpool-version-edit.js?v=2026070601"></script>
    ...
    function sameTask(daily, taskId) {
      return String(daily.taskId || daily.taskPoolId || daily.relatedTaskId || daily.poolTaskId || daily.task_id || '') === String(taskId);
    }
    ...
    for (const item of items) {
      const existed = db.dailyTasks.some((daily) => {
        return sameUser(daily) && sameTask(daily, item.taskId) && dailyDate(daily) === date;
      });
...
<link rel="stylesheet" href="/admin-taskpool-version-edit.css?v=2026070601">
          ...
          <body>
            <script>
              alert(${JSON.stringify('你今天已经填写过任务「' + title + '」，同一个任务一天只能填写一次。')});
              history.back();
            </script>
          <script src="/sidebar-bottom-font.js?v=2026070301"></script>
<script src="/intern-daily-multi-task.js?v=2026070308"></script>
...
<script src="/admin-taskpool-version-edit.js?v=2026070601"></script>
    ...
    for (const item of items) {
      ...
      const progressRaw =
        ...
        req.body.overallProgress !== undefined ? req.body.overallProgress :
        req.body.taskProgress !== undefined ? req.body.taskProgress :
        '';
      ...
      const daily = {
        ...
        taskId: item.taskId,
        taskPoolId: item.taskId,
        relatedTaskId: item.taskId,
        poolTaskId: item.taskId,
        ...
        problem: String(req.body.problem || req.body.issue || req.body.support || '').trim(),
        issue: String(req.body.problem || req.body.issue || req.body.support || '').trim(),
        tomorrowPlan: String(req.body.tomorrowPlan || req.body.plan || req.body.nextPlan || '').trim(),
        plan: String(req.body.tomorrowPlan || req.body.plan || req.body.nextPlan || '').trim(),

        progress: String(progressRaw || '').trim(),
        overallProgress: String(progressRaw || '').trim(),
...
<link rel="stylesheet" href="/admin-taskpool-version-edit.css?v=2026070601">
          ...
          <body>
            <script>
              alert('每一条任务都必须选择关联任务、填写今日工作内容和整体进度。');
              history.back();
            </script>
          <script src="/intern-daily-multi-task.js?v=2026070308"></script>
<script src="/intern-sidebar-order.js?v=2026070301"></script>
...
<script src="/admin-taskpool-version-edit.js?v=2026070601"></script>
      ...
      items.push({
        taskId,
        content,
        progress: String(Math.round(progressNum))
      });
    }
...
<script src="/admin-taskpool-version-edit.js?v=2026070601"></script>
    ...
    const problemText = String(req.body.problem || req.body.issue || req.body.support || '').trim();
...
function reportForm({ user, report = null, defaults = null, aiNote = '' }) {
  ...
  const data = {
    workContent: report?.workContent ?? defaults?.workContent ?? '',
  ...
  return layout({
    ...
    user,
    content: `<section class="page-title">
        <div><h1>${isEdit ? '查看 / 修改周报' : '填写本周周报'}</h1><p>周报可以随时修改，提交后老板可在管理端查看。</p></div>
      ...
      <section class="card">
        <form method="post" action="${formAction}" class="form grid-form">
          ...
          <label>需要老板支持<textarea name="supportNeeded" rows="3">${escapeHtml(data.supportNeeded)}</textarea></label>
          <div class="actions">
...
app.get('/intern/reports/new', requireLogin, requireIntern, async (req, res) => {
  ...
  // 如果这个人本周已经有周报，进入填写页时直接编辑原周报；再次提交会覆盖原结果，不新增第二条。
  if (existingReport) {
    const reportForEdit = req.query.ai === '1' ? { ...existingReport, ...defaults } : existingReport;
    return res.send(reportForm({ user: req.user, report: reportForEdit }));
  }
...
app.post('/intern/reports', requireLogin, requireIntern, (req, res) => {
  ...
  // 同一实习生、同一周报周期只允许一条记录；重复提交时覆盖旧记录，保留最后一次结果。
...
app.get('/admin/dashboard', requireLogin, requireAdmin, (req, res) => {
  ...
  const submittedRows = submittedInterns.length
    ? submittedInterns.map((u) => {
        ...
        return `<tr>
          ...
          <td>${tasks.length}</td>
          <td>${avgProgress === '-' ? '-' : progressBadge(avgProgress)}</td>
          <td>${lastUpdated ? escapeHtml(lastUpdated.slice(0, 16).replace('T', ' ')) : '-'}</td>
        </tr>`;
      }).join('')
    : '<tr><td colspan="5" class="empty">今日暂无已交人员。</td></tr>';
  ...
  const taskRows = todayTasks.length
    ? todayTasks.map((t) => {
        const intern = db.users.find((u) => u.id === t.userId) || { realName: t.userName || '-', position: t.position || '-' };
        return `<tr>
          <td>${escapeHtml(intern.realName)}</td>
          ...
          <td>${progressBadge(t.progress)}</td>
          <td><a class="link-button small" href="/admin/daily-tasks/${t.id}/edit?back=/admin/dashboard?date=${encodeURIComponent(selectedDate)}">编辑</a></td>
        </tr>`;
      }).join('')
    : '<tr><td colspan="7" class="empty">今日暂无每日任务记录。</td></tr>';
  ...
  res.send(layout({
    ...
    content: `<section class="page-title">
      ...
      <section class="card wide-card">
        ...
        <table>
          <thead><tr><th>姓名</th><th>职位</th><th>提交条数</th><th>平均进度</th><th>最后更新时间</th></tr></thead>
      ...
      <section class="card wide-card">
        <h2>今日任务明细</h2>
        <table class="daily-table today-detail-table">
          <thead><tr><th>姓名</th><th>职位</th><th>关联任务</th><th>今日工作内容</th><th>问题/支持</th><th>整体进度</th><th>操作</th></tr></thead>
...
app.get('/admin/reports/generate', requireLogin, requireAdmin, (req, res) => {
  const db = readDb();
  const interns = db.users.filter((u) => u.role === 'intern');
  const startDate = req.query.startDate || weekStartDefault();
  const endDate = req.query.endDate || weekEndDefault();
  const userId = req.query.userId || 'all';
  const success = req.query.success ? `<div class="alert success">已根据日报生成/覆盖 ${escapeHtml(req.query.success)} 份周报。老板账号可在周报管理中查看。</div>` : '';
  const skipped = req.query.skipped ? `<div class="alert error">有 ${escapeHtml(req.query.skipped)} 名实习生在该日期区间内没有日报，已自动跳过。</div>` : '';
  const internOptions = ['<option value="all">全部实习生</option>'].concat(interns.map((u) => `<option value="${escapeHtml(u.id)}" ${u.id === userId ? 'selected' : ''}>${escapeHtml(u.realName)}｜${escapeHtml(u.position)}</option>`)).join('');

  const generatedRows = getFilteredReports(db, { startDate, endDate, userId: userId === 'all' ? '' : userId }).slice(0, 20).map((r) => `<tr>
    <td>${escapeHtml(r.intern.realName)}</td>
    <td>${escapeHtml(r.intern.position)}</td>
    <td>${escapeHtml(r.weekStart)} 至 ${escapeHtml(r.weekEnd)}</td>
    <td>${statusBadge(r.status)}</td>
    <td>${escapeHtml((r.updatedAt || '').slice(0, 16).replace('T', ' '))}</td>
  </tr>`).join('') || '<tr><td colspan="5" class="empty">当前区间暂无已生成周报。</td></tr>';

  res.send(layout({
    title: '一键生成周报',
    user: req.user,
    content: `<section class="page-title">
      <div><h1>根据日期区间生成周报</h1><p>管理员可以选择实习生和日期区间，把该区间内的日报自动汇总为周报；同一人同一区间重复生成会覆盖旧结果。</p></div>
      <a class="ghost-link" href="/admin/daily-tasks">返回日报管理</a>
    </section>
    ${success}${skipped}
    <section class="card ai-card">
      <h2>一键生成</h2>
      <form method="post" action="/admin/reports/generate" class="filter-form compact-filter">
        <select name="userId">${internOptions}</select>
        <input type="date" name="startDate" value="${escapeHtml(startDate)}" required />
        <input type="date" name="endDate" value="${escapeHtml(endDate)}" required />
        <button class="primary" type="submit">一键生成周报</button>
      </form>
      <p class="muted">生成逻辑：读取所选日期区间内的日报，自动整理“工作内容、成果、问题、下周计划、需要支持”。生成后状态为“已提交”，老板可以查看。</p>
    </section>
    <section class="card wide-card">
      <div class="section-head">
        <h2>当前区间周报</h2>
        <a class="secondary" href="/admin/reports/export?startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}${userId !== 'all' ? `&userId=${encodeURIComponent(userId)}` : ''}">导出周报</a>
      </div>
      <table>
        <thead><tr><th>姓名</th><th>职位</th><th>周期</th><th>状态</th><th>更新时间</th></tr></thead>
...
app.post('/admin/reports/generate', requireLogin, requireAdmin, async (req, res) => {
  const db = readDb();
  const startDate = req.body.startDate || weekStartDefault();
  const endDate = req.body.endDate || weekEndDefault();
  const targetUsers = req.body.userId === 'all'
    ? db.users.filter((u) => u.role === 'intern')
    : db.users.filter((u) => u.role === 'intern' && u.id === req.body.userId);
  let successCount = 0;
  let skippedCount = 0;
  for (const intern of targetUsers) {
    const hasDailyTasks = db.dailyTasks.some((t) => t.userId === intern.id && t.date >= startDate && t.date <= endDate);
    if (!hasDailyTasks) {
      skippedCount += 1;
      continue;
    }
    const draft = await buildWeeklyDraftWithAI(db, intern, startDate, endDate);
    upsertWeeklyReportFromDraft(db, intern, startDate, endDate, draft, 'submitted');
    successCount += 1;
  }
  if (typeof syncTaskPoolProgressFromDailyTasks === 'function') syncTaskPoolProgressFromDailyTasks(db);
  writeDb(db);
  res.redirect(`/admin/reports/generate?success=${successCount}&skipped=${skippedCount}&startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}&userId=${encodeURIComponent(req.body.userId || 'all')}`);
});
...
app.get('/admin/reports/export', requireLogin, requireAdmin, (req, res) => {
  ...
  sendCsv(res, `周报导出_${today()}.csv`,
    ['姓名', '账号', '职位', '周期开始', '周期结束', '状态', '本周工作内容', '本周成果', '遇到的问题', '解决方案/思考', '下周计划', '需要老板支持', '提交时间', '更新时间'],
...
function bossV2SyncTaskProgress(db) {
  ...
  (db.taskPool || []).forEach((task) => {
    ...
    if (progress >= 100) {
      ...
      (Array.isArray(task.claimedByUserIds) && task.claimedByUserIds.length)
    ) {
      task.status = '已认领';
    } else {
      task.status = '待认领';
...
<body class="boss-v2-page">
  <aside class="boss-v2-sidebar">
    <div class="boss-v2-brand">
      <strong>解悠数字科技（南京）有限公司</strong>
      <span>实习生周报系统</span>
    </div>

    <nav class="boss-v2-nav-main">
      <div class="boss-v2-nav-title">核心功能</div>
      <a class="${active === 'weekly' ? 'active' : ''}" href="/boss/weekly-management">周报管理</a>
      <a class="${active === 'daily' ? 'active' : ''}" href="/boss/dashboard">日报看板</a>
    </nav>

    <div class="boss-v2-nav-bottom">
      <div class="boss-v2-nav-title">系统管理</div>
      <a href="/change-password">修改密码</a>
      <form method="post" action="/logout">
        <button type="submit">退出登录</button>
      </form>
      <div class="boss-v2-identity">老板 · 老板</div>
    </div>
  </aside>

  <main class="boss-v2-main">
    <section class="boss-v2-header">
      <div>
...
app.get('/boss/dashboard', requireLogin, requireBoss, (req, res) => {
  ...
  const users = bossV2UserMap(db);
  const tasks = (db.taskPool || []).slice().sort((a, b) => {
    const pa = bossV2Percent(a.progress) ?? 0;
    const pb = bossV2Percent(b.progress) ?? 0;
  ...
  const weeklyReports = (
    ...
    const ak = `${a.submittedAt || a.createdAt || ''}|${a.id || ''}`;
    const bk = `${b.submittedAt || b.createdAt || ''}|${b.id || ''}`;
    return bk.localeCompare(ak);
  });

  const totalInterns = (db.users || []).filter((u) => u.role === 'intern').length;
  const totalTasks = tasks.length;
  const doingTasks = tasks.filter((t) => {
    const p = bossV2Percent(t.progress) ?? 0;
    return p > 0 && p < 100;
  }).length;
  const doneTasks = tasks.filter((t) => (bossV2Percent(t.progress) ?? 0) >= 100).length;
  const dailyCount = dailyTasks.length;

  const taskRows = tasks.map((task) => {
    const progress = bossV2Percent(task.progress) ?? 0;
    const assignees = bossV2TaskAssigneeNames(db, task).join('、') || '-';
    const logs = dailyTasks
      .filter((daily) => bossV2DailyMatchesTask(daily, task))
      .sort((a, b) => bossV2SortKey(b).localeCompare(bossV2SortKey(a)));

    const latest = logs[0];
    const progressClass = progress >= 100 ? 'done' : progress > 0 ? 'doing' : 'zero';

    return `
      <tr>
        <td class="boss-v2-title-cell">
          <a href="/boss/task-daily/${encodeURIComponent(task.id)}">
            ${bossV2Esc(task.title || '-')}
          </a>
        </td>
        <td>${bossV2Esc(task.department || '-')}</td>
        <td>${bossV2Esc(assignees)}</td>
        <td>
          <a class="boss-v2-progress-link" href="/boss/task-daily/${encodeURIComponent(task.id)}">
            <strong>${progress}%</strong>
            <span class="boss-v2-progress-bar">
              <i class="${progressClass}" style="width:${progress}%"></i>
            </span>
          </a>
        </td>
        <td>${bossV2Esc(latest?.date || '-')}</td>
        <td class="boss-v2-content-cell">${bossV2Esc(latest?.content || latest?.workContent || '-')}</td>
      </tr>
    `;
  }).join('');

  const dailyRows = dailyTasks.slice(0, 80).map((daily) => {
    ...
    return `
      <tr>
        ...
        <td class="boss-v2-title-cell">${bossV2Esc(bossV2DailyTaskTitle(daily) || '-')}</td>
        <td><span class="boss-v2-percent-pill">${progress}%</span></td>
        <td class="boss-v2-content-cell">${bossV2Esc(daily.content || daily.workContent || '-')}</td>
        <td>
          <a class="boss-v2-small-link" href="/boss/daily-detail/${encodeURIComponent(daily.id)}">
            查看详情
          </a>
        </td>
      </tr>
  ...
  const weeklyRows = weeklyReports.slice(0, 20).map((report) => {
    const name = bossV2UserName(db, report.userId, report.userName || report.realName || '-');
    const user = users[String(report.userId || '')] || {};
    const period = report.period || report.week || `${report.weekStart || ''} 至 ${report.weekEnd || ''}`;
    const status = report.status || '已提交';

    return `
      <tr>
        <td>${bossV2Esc(name)}</td>
        <td>${bossV2Esc(user.position || report.position || '-')}</td>
        <td>${bossV2Esc(period || '-')}</td>
        <td><span class="boss-v2-status">${bossV2Esc(status)}</span></td>
        <td>${bossV2Esc(report.submittedAt || report.createdAt || '-')}</td>
        <td>
          <a class="boss-v2-small-link" href="/boss/reports/${encodeURIComponent(report.id)}">
            查看
          </a>
        </td>
      </tr>
  ...
  const body = `
    ...
    <section class="boss-v2-card">
      <div class="boss-v2-card-head">
        <div>
          <h2>任务进度看板</h2>
          <p>多人领取同一任务时，整体进度按每个人最新日报进度中最慢的进度显示。</p>
        </div>
      </div>
      ...
      <div class="boss-v2-table-wrap">
        <table class="boss-v2-table">
          ...
          <tbody>
            ${taskRows || '<tr><td colspan="6">暂无任务数据</td></tr>'}
          </tbody>
        </table>
      </div>
    </section>

    <section class="boss-v2-card" id="daily">
      <div class="boss-v2-card-head">
        <div>
          <h2>每个人的日报</h2>
          <p>展示实习生每日填写的工作内容，点击详情可查看完整日报。</p>
        </div>
      </div>
      ...
      <div class="boss-v2-table-wrap">
        <table class="boss-v2-table">
          ...
          <tbody>
            ${dailyRows || '<tr><td colspan="7">暂无日报数据</td></tr>'}
          </tbody>
        </table>
      </div>
    </section>

    <section class="boss-v2-card" id="weekly">
      <div class="boss-v2-card-head">
        <div>
          <h2>周报管理</h2>
          <p>保留老板查看周报的入口，页面风格与日报看板统一。</p>
        </div>
        <a class="boss-v2-primary-link" href="/boss/reports">进入完整周报列表</a>
      </div>
      ...
      <div class="boss-v2-table-wrap">
        <table class="boss-v2-table">
          ...
          <tbody>
            ${weeklyRows || '<tr><td colspan="6">暂无周报数据</td></tr>'}
          </tbody>
        </table>
      </div>
    </section>
  ...
  res.send(bossV2Layout({
    title: '工作台',
    subtitle: '精简查看周报、日报、任务进度和每日更新内容。',
    active: 'daily',
    body
  }));
});
  ...
  res.send(layout({
    ...
    content: `<section class="page-title">
      ...
      <section class="card report-detail">
        ...
        <h2>需要老板支持</h2><p>${nl2br(report.supportNeeded || '无')}</p>
      </section>
      <section class="card">
        <h2>老板反馈</h2>
...
function syncTaskPoolProgressFromDailyTasks(db) {
  ...
  db.taskPool.forEach((task) => {
    ...
    const latestDaily = matchedDailyTasks[matchedDailyTasks.length - 1];
    const latestProgress = clampProgress(latestDaily.progress);

    if (latestProgress === null) {
      return;
    }

    task.progress = latestProgress;

    /*
     * 任务内部状态也同步，但任务总表页面展示仍然显示百分比。
     */
    if (latestProgress >= 100) {
      ...
      const hasAssignee = taskUserIds(task).length > 0 || Boolean(task.assigneeName);
      task.status = hasAssignee ? '已认领' : '待认领';
    }

    task.progressUpdatedFromDailyId = latestDaily.id || '';
    task.progressUpdatedFromDailyDate = latestDaily.date || '';
...
app.get('/boss/weekly-management', requireLogin, requireBoss, (req, res) => {
  ...
  function userName(report) {
    const user = userById(report.userId);
    return user.realName || report.userName || report.realName || '-';
  }

  function userPosition(report) {
    const user = userById(report.userId);
    return user.position || report.position || '-';
  }

  function periodText(report) {
    return report.period || report.week || `${report.weekStart || ''} 至 ${report.weekEnd || ''}` || '-';
  }
  ...
  function statusText(status) {
    ...
    if (s === 'returned' || s === '退回' || s === '已退回') return '已退回';
    if (s === 'submitted' || s === '已提交') return '已提交';
    if (s === 'draft' || s === '草稿') return '草稿';

    return s || '已提交';
  }

  const nameKeyword = String(req.query.name || '').trim();
  const statusKeyword = String(req.query.status || '').trim();

  let reports = (db.weeklyReports || db.reports || []).slice();

  if (nameKeyword) {
    reports = reports.filter((r) => userName(r).includes(nameKeyword));
  }

  if (statusKeyword) {
    reports = reports.filter((r) => statusText(r.status) === statusKeyword);
  }
  ...
  const internsCount = (db.users || []).filter((u) => u.role === 'intern').length;
  const totalCount = reports.length;
  const submittedCount = reports.filter((r) => statusText(r.status) === '已提交').length;
  const approvedCount = reports.filter((r) => statusText(r.status) === '已通过').length;
  const returnedCount = reports.filter((r) => statusText(r.status) === '已退回').length;

  const rows = reports.map((report) => {
    const status = statusText(report.status);
    const cls = status === '已通过' ? 'approved' : status === '已退回' ? 'returned' : 'submitted';

    return `
      <tr>
        <td>${esc(userName(report))}</td>
        <td>${esc(userPosition(report))}</td>
        <td>${esc(periodText(report))}</td>
        <td><span class="boss-v2-status ${cls}">${esc(status)}</span></td>
        <td>${esc(report.submittedAt || report.createdAt || '-')}</td>
        <td>
          <a class="boss-v2-small-link" href="/boss/reports/${encodeURIComponent(report.id)}">查看</a>
        </td>
      </tr>
    `;
  }).join('');
...
<body class="boss-v2-page">
  <aside class="boss-v2-sidebar">
    ...
    <nav class="boss-v2-nav-main">
      <div class="boss-v2-nav-title">核心功能</div>
      <a class="active" href="/boss/weekly-management">周报管理</a>
      <a href="/boss/dashboard">日报看板</a>
    </nav>
  ...
  <main class="boss-v2-main">
    ...
    <section class="boss-v2-card">
      ...
      <form class="boss-v2-filter" method="get" action="/boss/weekly-management">
        <input name="name" value="${esc(nameKeyword)}" placeholder="按姓名筛选" />
        <select name="status">
          <option value="">全部状态</option>
          <option value="已提交" ${statusKeyword === '已提交' ? 'selected' : ''}>已提交</option>
          <option value="已通过" ${statusKeyword === '已通过' ? 'selected' : ''}>已通过</option>
          <option value="已退回" ${statusKeyword === '已退回' ? 'selected' : ''}>已退回</option>
        </select>
        <button type="submit">筛选</button>
        <a href="/boss/weekly-management">重置</a>
      </form>
    </section>
    ...
    <section class="boss-v2-card">
      ...
      <div class="boss-v2-table-wrap">
        <table class="boss-v2-table">
          ...
          <tbody>
            ${rows || '<tr><td colspan="6">暂无周报数据</td></tr>'}
          </tbody>
        </table>
      </div>
    </section>
...
app.listen(PORT, HOST, () => {
  console.log(`解悠数字科技（南京）有限公司实习生周报系统已启动：`);
